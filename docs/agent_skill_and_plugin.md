...
The repository contains a Draft Codex plugin scaffold named `pe-version-info` (hyphenated because Codex plugin/Skill identifiers require it). The Rust project and GitHub repository use `pe_version_info` (underscored) according to the Tinkora repository naming convention. The scaffold is not yet a release artifact or Agent-callable transport.
...
- converting PNG/JPEG/ICO assets into a Windows executable icon. SVG/PDF may
  trigger only after the installed `pevi` build explicitly supports them.
...
6. Run `pevi plan --format json` and summarize the requested changes; do not present the summary as a stable field-by-field diff.
7. Refuse to overwrite the input unless the user explicitly requests in-place mode and confirms signature consequences.
8. Run `pevi apply` to a separate output by default. An explicit mutation request
   can authorize a new separate output; ask again for overwrite, in-place,
   certificate-table, or underspecified operations.
9. Run `pevi verify --config ... --format json`.
10. Report output path, hashes, verified version request, icon
    format/renderer/fit/background/sizes/crop state, and signature state from
    `apply.data`; report `matches` and the fresh inspection from `verify.data`.
...
- An unrequested `[version]` or `[icon]` section is omitted because its presence
  authorizes mutation of that resource type.
...
The actual manifest is generated by the official `plugin-creator` workflow. Do not publish the Draft scaffold as an installable release until the binary and Skill are tested together in hosted CI.
