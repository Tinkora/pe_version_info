...
An MCP server and UI are documented follow-up work; they are not present in the
current repository layout or plugin manifest.
...
5. Merge only configured values. Preserve unrelated string keys unless `preserve_unknown_strings = false` is explicitly configured.
...
11. Report signature state, input/output SHA-256, verified version requests,
    icon conversion metadata, and warnings.
...
| SVG (follow-up) | `resvg`/`usvg` | render at each target size; no crop | multi-resolution ICO frames |
| PDF (follow-up) | PDFium through `pdfium-render` | render selected page, default page 1; no crop | multi-resolution ICO frames |

“Any format” means that the configuration boundary is extensible, not that an
unknown extension should be forced through an image decoder. The first release
candidate enables PNG, JPEG, and ICO only. SVG and PDF remain follow-up
features. PDFium is a native runtime and must be distributed or discovered per
target; its licensing and binary provenance must be documented in
`THIRD_PARTY_NOTICES.md` before enabling PDF in a release build.
...
- Explicit renderer name and icon target sizes in the report.
