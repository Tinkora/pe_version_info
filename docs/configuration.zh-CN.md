...
首个发布候选支持 `en-US`/`1200` 和 PNG、JPEG、ICO。SVG 与 PDF 在后续 feature gate 完成前作为不支持格式失败。未知语言、未知格式和不符合约束的尺寸必须报错，不能静默回退。

## 2. 安全规则

输入和输出不能相同，除非同时指定 `--in-place --confirm-in-place`。已签名输入必须同时指定允许标志和“确认签名失效”标志。默认保留未知 VERSIONINFO 字段及无关资源；更新主图标时会先移除旧主图标及其不再共享的资源，默认不裁剪图标。
